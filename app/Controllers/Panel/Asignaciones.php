...
class Asignaciones extends BaseController
{
    ...
    {
        $session = session();
        if (!Permisos::is_rol_permitido(TAREA_PSICOLOGO_ASIGNACIONES, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
            $this->permitido = false;
        } //end if rol permitido
        else {
            $session->tarea_actual = TAREA_PSICOLOGO_ASIGNACIONES;
    ...
    {
        if ($this->permitido) {
            return $this->crear_vista("panel/ver_asignaciones", $this->cargar_datos());
        } //end if rol permitido
        else {
