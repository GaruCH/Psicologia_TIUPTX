...
class Historial_psicologo_citas extends BaseController
{
    ...
    {
        $session = session();
        if (!Permisos::is_rol_permitido(TAREA_PSICOLOGO_CITAS_HISTORIAL, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
            $this->permitido = false;
        } //end if rol permitido
        else {
            $session->tarea_actual = TAREA_PSICOLOGO_CITAS_HISTORIAL;
    ...
    {
        if ($this->permitido) {
            return $this->crear_vista("panel/psicologo_historial_citas", $this->cargar_datos());
        } //end if rol permitido
        else {
    ...
    public function generar_datatable_historial_citas()
    {
        if ($this->permitido) {
            ...
            $citas = $tabla_citas->datatable_historial_citas($session->id_usuario);
            $data = array();
            $count = 0;
